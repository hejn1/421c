...
x_sq = square((pi/12)*t_sq,0.5)/2+0.5; %semnalul original
...
for k = -N:N %k este variabila dupa care se realizeaza suma
    x_s = x_sq; %x_s e semnalul realizat dupa formula SF data
    ...
    for i = 1:length(t_sq)-1
        X(k+N+1) = X(k+N+1) + (t_sq(i+1)-t_sq(i)) * (x_s(i)+x_s(i+1))/2; %reconstructia folosind coeficientii
    end
end
